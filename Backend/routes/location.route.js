...
// Add these debug endpoints to your routes/location.route.js
    ...
    const sampleBuses = [
      {
        ...
        route: [
          ...
          {
            type: "Point",
            coordinates: [77.2090, 28.6139], // Center
            timestamp: new Date()
          }
        ],
        status: "Active",
      ...
      },
      {
        ...
        route: [
          ...
          {
            type: "Point",
            coordinates: [77.3910, 28.5355], // South Delhi
            timestamp: new Date()
          }
        ],
        status: "Active",
...
locationRoute.get('/debug/test-route-search', async (req, res) => {
  try {
    ...
    for (const route of testRoutes) {
      try {
        const { getBusesAlongRoute } = await import('../controllers/Location.controller.js');
        ...
        const mockRes = {
          ...
          status: (code) => ({ json: (data) => { mockResult = { ...data, statusCode: code }; } })
        };
